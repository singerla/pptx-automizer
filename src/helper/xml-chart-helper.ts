...
export class XmlChartHelper {
  static setChartData(chart: XMLDocument, workbook: Workbook, data:ChartData): void {
    ...
    for (const c in data.categories) {
      for (const s in data.categories[c].values) {
        ...
        series[s].getElementsByTagName('c:v')[0]
          .firstChild.textContent = data.series[s].label;
  ...
  static setWorkbookData(workbook: Workbook, data:ChartData): void {
    ...
    for (const c in data.categories) {
      const r = Number(c) + 1;
      const stringId = XmlChartHelper.appendSharedString(workbook.sharedStrings, data.categories[c].label);
    ...
    for (const s in data.series) {
      ...
      const stringId = XmlChartHelper.appendSharedString(workbook.sharedStrings, data.series[s].label);
